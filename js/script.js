(function($) {
	...
	// dropdown menu
	let mobileWidth = 992;
	let navcollapse = $('.navigation li.dropdown');
	...
	function enableMasonry() {
		if($('.sortable-masonry').length){
			...
			winDow.bind('resize', function(){
				let selector = $filter.find('li.active').attr('data-filter');
	...
	function customTabProductPageTab () {
	    if ($('.custom-tab-title').length) {
	        let tabWrap = $('.tab-details-content');
	        let tabClicker = $('.custom-tab-title ul li');
	...
	function progressBarConfig () {
	  	let progressBar = $('.progress');
	  		if(progressBar.length) {
	        	...
	        	let progressValue = Self.data('value');
	...
	function priceFilter () {
		if($('.range-slider-price').length){
			...
			let priceRange = document.getElementById('range-slider-price');
			...
			let limitFieldMin = document.getElementById('min-value-rangeslider');
			let limitFieldMax = document.getElementById('max-value-rangeslider');
